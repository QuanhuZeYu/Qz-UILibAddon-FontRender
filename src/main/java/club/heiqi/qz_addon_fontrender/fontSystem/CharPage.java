...
public class CharPage {
    ...
    public CharPage(String markInfo, Font font) {
        ...
        this.markInfo = fontName+"-"+markInfo+"-"+count;
        this.font = font.makeWithSize(charSize*0.95f);
    ...
    public static void autoAddNormalChar(String c, List<Font> fonts) {
        ...
        for (Font font : fonts) {
            ...
            if (glyph == 0) continue;
                ...
                CharPage page = findPageWithFont(font).stream().filter(p -> !p.isFull()).findFirst().orElse(null);
